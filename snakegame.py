...
def fim_de_jogo():
    canvas.delete(ALL)
    canvas.create_text(canvas.winfo_width()/2, canvas.winfo_height()/2, font=('consolas', 70), text='PERDEU MENÓ', fill='red', tag='fimdejogo')
    canvas.create_text(canvas.winfo_width()/2, canvas.winfo_height()/2+60, font=('consolas', 30), text='Aperte espaço para reiniciar', fill='white', tag='fimdejogo')
    window.bind('<space>', reiniciar_jogo)
def reiniciar_jogo(event):
...
label = Label(window, text='Pontuação: 0', font=('Arial', 40))
label.pack()
...
window.geometry(f'{largura_janela}x{altura_janela}+{int(x)}+{int(y)}')
